...
ciiLNAGoodness = zeros(tRangeLength, 5001);
ciiPlefkaGoodness = zeros(tRangeLength, 5001);

%transients
ciiOrigTransient = zeros(tRangeLength, 5001);
ciiLNATransient = zeros(tRangeLength, 5001);
ciiPlefkaTransient = zeros(tRangeLength, 5001);
 ...
 for T=1:tRangeLength
    ...
    myOrigCii = squeeze(mean(myOrigData, 1));
    myLNACii = squeeze(mean(myLNAData, 1));
    myPlefkaCii = squeeze(mean(myPlefkaData, 1));

    ciiLNAGoodness(T,:) = goodnessOfFit(myOrigCii, myLNACii, n)';
    ciiPlefkaGoodness(T,:) = goodnessOfFit(myOrigCii, myPlefkaCii, n)';
    
    figure;
    plot(ciiLNAGoodness(T,:), 'g');
    hold on;
    plot(ciiPlefkaGoodness(T,:));
    title('Cii goodness of fit');
    legend('LNA','Plefka')
    hold off;
    
    ciiOrigTransient(T,:) = transient(myOrigCii, n);
    ciiLNATransient(T,:) = transient(myLNACii, n);
    ciiPlefkaTransient(T,:) = transient(myPlefkaCii, n);
...
figure;
hold on;
title('Transients Original Cii');
for ii=1:tRangeLength
plot(ciiOrigTransient(ii,:)','color',rand(1,3));
end
hold off;

figure;
hold on;
title('Transients LNA Cii');
for ii=1:tRangeLength
plot(ciiLNATransient(ii,:)','color',rand(1,3));
end

figure;
hold on;
title('Transients Plefka Cii');
for ii=1:tRangeLength
plot(ciiPlefkaTransient(ii,:)','color',rand(1,3));
end
